...
export default function InvitePage() {
  ...
  return (
    <main className="app-page-bg min-h-screen px-6 py-10 text-slate-950">
      <section className="mx-auto grid max-w-4xl gap-6">
        <div>
          <Link className="text-sm font-semibold text-blue-700 hover:text-blue-900" href="/">
            Nexterview
          </Link>
          <h1 className="mt-4 text-3xl font-semibold tracking-tight text-slate-950">Candidate invite</h1>
        </div>

        <div className="rounded-card border border-white/80 bg-white p-6 shadow-elevated">
          {isLoading ? <LoadingState label="Loading candidate invite" rows={2} /> : null}
          {error ? <ErrorState message={error} /> : null}
          {invite ? (
            <div className="grid gap-5">
              <div>
                <p className="text-sm text-slate-600">Invited candidate</p>
                <p className="mt-1 font-medium text-slate-950">{invite.candidate_name || invite.candidate_email || "Any candidate in the organization"}</p>
                {invite.candidate_email ? <p className="mt-1 text-sm text-slate-500">{invite.candidate_email}</p> : null}
              </div>
              <div>
                <h2 className="text-2xl font-semibold text-slate-950">{invite.interview.role_title}</h2>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {invite.interview.stack.map((item) => (
                    <span className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs text-slate-700" key={item}>
                      {item}
                    </span>
                  ))}
                </div>
              ...
              {!isAuthLoading && !user && invite.interview.is_ready ? (
                <Link
                  className="inline-flex h-11 w-fit items-center justify-center rounded-lg border border-blue-700 bg-blue-600 px-4 text-sm font-semibold text-white shadow-sm transition hover:bg-blue-700"
                  href={loginHref}
                >
                  Log in to start
                </Link>
